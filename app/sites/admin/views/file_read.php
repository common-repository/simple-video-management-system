...
$file = wp_remote_get(  plugins_url('../../../../readme.txt', __FILE__));

 $ar = array("include"=>"Changelog");
 $new_array = array();
 $new_array['name'] = "simple-auto-webinar";
 $new_array['file'] = $file['body'];
$readm =  readme_parser_SVMS($ar,"",$new_array);

echo $readm;

function readme_parser_SVMS( $paras = '', $content = '',$new_array ) {
    ...
    if ( is_numeric( $cache ) ) {
        ...
        $ignore = sas_arp_get_list( $ignore, ',,' );
        $mirror = sas_arp_get_list( $mirror, ',,' );
		...
		$show_links = false;
		if ( '' != $include ) {
			if ( sas_arp_is_it_excluded( 'links', $include ) ) { $show_links = true; }
		} else {
			if ( !sas_arp_is_it_excluded( 'links', $exclude ) ) { $show_links = true; }
		}

		// Ensure EXCLUDE and INCLUDE parameters aren't both included

		if ( ( '' != $exclude ) && ( '' != $include ) ) {
			return sas_arp_report_error( __( 'INCLUDE and EXCLUDE parameters cannot both be specified', 'wp-readme-parser' ), 'Plugin README Parser', false );
		}

        // Work out filename and fetch the contents
        //$plugin_url = "Simple Auto Webinar";
        //$file_data = sas_arp_get_readme( $plugin_url, $version );
        $file_data = $new_array;
        /*echo "<pre>";
            print_r($file_data);
        echo "</pre>"; */
        // Ensure the file is valid
            ...
            for ( $i = 0; $i < $count; $i++ ) {
                ...
                if ( '=== ' == substr( $file_array [$i ], 0, 4 ) ) {
                    ...
                    $section = sas_arp_get_section_name( $file_array[ $i ], 1 );
                } else {
                    if ( '== ' == substr( $file_array[ $i ], 0, 3 ) ) {
                        $file_array[ $i ] = str_replace( '==', '##' , $file_array[ $i ] );
                        $section = sas_arp_get_section_name( $file_array[ $i ], 2 );
                    } else {
				...
				if ( '' != $include ) {
	                ...
	                if ( sas_arp_is_it_excluded( $section, $include ) ) {
					...
					if ( sas_arp_is_it_excluded( $section, $exclude ) ) {
						$add_to_output = false;
					} else {
						if ( $section != $prev_section ) {
							if ( $div_written ) { $file_combined .= '</div>' . $crlf; }
							$file_combined .= $crlf . '<div markdown="1" class="np-' . esc_attr(htmlspecialchars( str_replace( ' ', '-', strtolower( $section ) ) )) . '">' . $crlf;
							$div_written = true;
						}
					}
				}
                ...
                if ( ( $links == strtolower( $section ) ) && ( $section != $prev_section ) ) {
                    ...
                    if ( $show_links ) { $file_array[ $i ] = sas_arp_display_links( $download, $target, $nofollow, $version, $mirror, $plugin_name ) . $file_array[ $i ]; }
                }
                ...
                if ( $add_to_output ) {
                    ...
                    if ( ( 'Contributors:' == substr( $file_array[ $i ], 0, 13 ) ) or ( 'Donate link:' == substr( $file_array[ $i ], 0, 12 ) ) or ( 'Tags:' == substr( $file_array[ $i ], 0, 5 ) ) or ( 'Requires at least:' == substr( $file_array[ $i ], 0, 18 ) ) or ( 'Tested up to:' == substr( $file_array[ $i ], 0, 13 ) ) or ( 'Stable tag:' == substr( $file_array[ $i ], 0, 11 ) ) or ( 'License URI:' == substr( $file_array[ $i ], 0, 12 ) ) or ( 'License:' == substr( $file_array[ $i ], 0, 8 ) ) ) {
                        ...
                        if ( sas_arp_is_it_excluded( 'meta', $exclude ) ) { $add_to_output = false; }

                        if ( ( 'Requires at least:' == substr( $file_array[ $i ], 0, 18 ) ) && ( sas_arp_is_it_excluded( 'requires', $exclude ) ) ) { $add_to_output = false; }

                        if ( ( 'Tested up to:' == substr( $file_array[ $i ], 0, 13 ) ) && ( sas_arp_is_it_excluded( 'tested', $exclude ) ) ) { $add_to_output = false; }

                        // Show contributors and tags using links to WordPress pages

                        if ( 'Contributors:' == substr( $file_array[ $i ], 0, 13 ) ) {
                            if ( sas_arp_is_it_excluded( 'contributors', $exclude ) ) {
                                $add_to_output = false;
                            } else {
                                $file_array[ $i ] = substr( $file_array[ $i ], 0, 14 ) . sas_arp_strip_list( substr( $file_array[ $i ], 14 ), 'c', $target, $nofollow );
                            }
                        }
                        if ( 'Tags:' == substr( $file_array[ $i ], 0, 5 ) ) {
                            if ( sas_arp_is_it_excluded( 'tags', $exclude ) ) {
                                $add_to_output = false;
                            } else {
                                $file_array[ $i ] = substr( $file_array[ $i ], 0, 6 ) . sas_arp_strip_list( substr( $file_array[ $i ], 6 ), 't', $target, $nofollow );
                            }
                        }
                        ...
                        if ( 'Donate link:' == substr( $file_array[ $i ], 0, 12 ) ) {
                            if ( sas_arp_is_it_excluded( 'donate', $exclude ) ) {
                                $add_to_output = false;
                            } else {
                                $text = substr( $file_array[ $i ], 13 );
                                $file_array[ $i ] = substr( $file_array[ $i ], 0, 13 ) . '<a href="' . esc_url($text) . '">' . $text . '</a>';
                            }
                        }
                        ...
                        if ( 'License URI:' == substr( $file_array[ $i ], 0, 12 ) ) {
                            if ( sas_arp_is_it_excluded( 'license uri', $exclude ) ) {
                                $add_to_output = false;
                            } else {
                                $text = substr( $file_array[ $i ], 13 );
                                $file_array[ $i ] = substr( $file_array[ $i ], 0, 13 ) . '<a href="' . esc_url($text) . '">' . $text . '</a>';
                        ...
                        if ( 'Stable tag:' == substr( $file_array[ $i ], 0, 11 ) ) {
                            if ( sas_arp_is_it_excluded( 'stable', $exclude ) ) {
                                $add_to_output = false;
                            } else {
                                $file_array[ $i ] = substr( $file_array[ $i ], 0, 12 ) . '<a href="' . esc_url($download).'" style="max-width: 100%;">' . $version . '</a>';
                            }
                        }
                ...
                if ( ( 'Screenshots' == $section ) && ( $add_to_output ) && ( '' != $screenshot_url ) ) {
                    if ( substr( $file_array[ $i ], 0, strlen( $screenshot ) + 2 ) == $screenshot . '. ' ) {
                        ...
                        $ext = sas_arp_check_img_exists( $this_screenshot, 'png' );
                        if ( !$ext ) { $ext = sas_arp_check_img_exists( $this_screenshot, 'gif' ); }
                        if ( !$ext ) { $ext = sas_arp_check_img_exists( $this_screenshot, 'jpg' ); }
                        if ( !$ext ) { $ext = sas_arp_check_img_exists( $this_screenshot, 'jpeg' ); }
                        $this_screenshot .= $ext;
                        ...
                        if ( !$ext ) {
                            ...
                            $file_array[ $i ] = sas_arp_report_error( sprintf( __( 'Could not find %s image file', 'wp-readme-parser' ), 'screenshot-' . $screenshot ), 'Plugin README Parser', false );

                        } else {

                            $file_array[ $i ] = '<img src="' . esc_url($this_screenshot) . '" alt="' . esc_attr($plugin_title) . ' Screenshot ' . esc_attr($screenshot) . '" title="' . esc_attr($plugin_title) . ' Screenshot ' . $screenshot . '" class="np-screenshot' . esc_attr($screenshot) . '" /><br />' . $crlf . '*' . substr( $file_array[ $i ], strlen( $screenshot ) + 2 ) . '*';
            ...
            if ( ( $show_links ) && ( 'bottom' == $links ) ) { $file_combined .= sas_arp_display_links( $download, $target, $nofollow, $version, $mirror, $plugin_name ); }
