...
function sas_arp_get_readme( $plugin_url, $version = '' ) {
    ...
    $file_data = sas_arp_get_file( $plugin_url );
...
function sas_arp_is_it_excluded( $tofind, $exclude ) {
...
function sas_arp_get_section_name( $readme_line, $start_pos ) {
...
function sas_arp_display_links( $download, $target, $nofollow, $version, $mirror, $plugin_name ) {
...
function sas_arp_check_img_exists( $filename, $ext ) {
...
function sas_arp_strip_list( $list, $type, $target, $nofollow ) {
...
function sas_arp_get_file( $filein, $header = false ) {
...
function sas_arp_get_list( $input, $seperator = '' ) {     // Version 1.2
...
function sas_arp_report_error( $error, $plugin_name, $echo = true ) {

	$output = '<p style="color: #f00; font-weight: bold;">' . esc_attr($plugin_name) . ': ' . esc_attr($error) . "</p>\n";
